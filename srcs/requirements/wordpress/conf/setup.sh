...
if [ ! -f "wp-settings.php" ]; then
	echo " ✅ WORDPRESS STAFF"
	php83 -d memory_limit=256M /usr/local/bin/wp core download --allow-root
fi

sleep_time=1
while ! php83 -d memory_limit=256M -r "mysqli_report(MYSQLI_REPORT_OFF); \$mysqli = new mysqli('$MYSQL_HOST', '$MYSQL_USER', '$MYSQL_PWD', '$MYSQL_DB'); if (\$mysqli->connect_error) exit(1); \$mysqli->close();" >/dev/null 2>&1; do
	sleep $sleep_time
...
if [ ! -f "wp-config.php" ]; then
	php83 -d memory_limit=256M /usr/local/bin/wp config create --allow-root \
		--dbname="$MYSQL_DB" \
		--dbuser="$MYSQL_USER" \
		--dbpass="$MYSQL_PWD" \
		--dbhost="$MYSQL_HOST"
